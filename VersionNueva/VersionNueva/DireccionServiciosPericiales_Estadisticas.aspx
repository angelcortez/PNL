<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Site.Master" CodeBehind="DireccionServiciosPericiales_Estadisticas.aspx.cs" Inherits="AtencionTemprana.DireccionServiciosPericiales_Estadisticas" %>
<%@ Register assembly="AjaxControlToolkit" namespace="AjaxControlToolkit" tagprefix="asp" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
<style type="text/css">
     .hidden
     {
         display:none;
     }
    .style3
        {
            width: 988px;
        }
    .style4
    {
        height: 85px;
    }
</style>
</asp:Content>
<asp:Content ID="Content2" runat="server" 
    contentplaceholderid="ContentPlaceHolder1">
         
        
<div id="main-wrapper">
     <asp:ScriptManager ID="ScriptManager1" runat="server">
    </asp:ScriptManager> 
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
    <ContentTemplate>
      <table style="width: 100%;">
        <tr>
            <td>
                
                <asp:Label ID="UNDD" runat="server" Font-Bold="True" Font-Size="Medium" 
                    ForeColor="#006600"></asp:Label>
            </td>
            <td>
                &nbsp;</td>
            <td>
                &nbsp;</td>
            <td align="right">
                <asp:Label ID="lblFecha" runat="server" Font-Bold="True" ForeColor="Black"></asp:Label>
            </td>
        </tr>
     
      
         <tr>
             <td>
                 <asp:Label ID="LBLUSUARIO" runat="server" Font-Bold="True" ForeColor="#666666" 
                     style="text-transform :uppercase"></asp:Label>
             </td>
             <td>
                 &nbsp;</td>
             <td>
                 &nbsp;
             </td>
             <td align="right">
                 &nbsp;</td>
         </tr>
     
      
         <tr>
             <td>
                 <asp:Label ID="PUESTO" runat="server" Font-Bold="True" ForeColor="#666666" 
                     style="text-transform :uppercase"></asp:Label>
                     <asp:Label ID="IdUsuario" runat="server" Visible="False"></asp:Label>
                 <asp:Label ID="IdUnidad" runat="server" Visible="False"></asp:Label>
                 <asp:Label ID="IdMunicipio" runat="server" Visible="False"></asp:Label>
                 <asp:Label ID="IdOrden" runat="server" Visible="false" ></asp:Label>
                 <asp:Label ID="IDtIPOuNIDAD" runat="server" Text=" " Visible="false"></asp:Label>
             </td>
             <td>
                 &nbsp;</td>
             <td>
                 &nbsp;</td>
             <td align="right">
                 &nbsp;</td>
         </tr>
     
      
    </table>

        <table style="width: 100%;" >
            
            <tr>
                <td>
                   <asp:ImageButton ID="Image2" runat="server" Height="18px" 
                        ImageUrl="img/home.png" onclick="Image2_Click"/>
                            <asp:Label ID="lblInicio" Text="INICIO" runat="server" Font-Bold="True"  Font-Size="18px" ForeColor="#666666" style="text-transform :uppercase" class="margen"></asp:Label>    

                    
                            <asp:ImageButton ID="Image3" runat="server" Height="18px" 
                            ImageUrl="img/graph.jpg" onclick="Image3_Click"  />
                            <asp:Label ID="Label1" Text="ESTADÍSTICAS" runat="server" 
                        Font-Bold="True" Font-Size="18px" ForeColor="#666666" 
                        style="text-transform :uppercase" class="margen"></asp:Label>    
                    
                </td>
                
            </tr>
             
            <tr>
                <td>
                    
                    <asp:Button ID="Button1" runat="server" onclick="Button1_Click" 
                        Text="CARGA DE TRABAJO DE PERITOS" Height="21px" Width="234px" />
                    &nbsp;&nbsp; &nbsp;&nbsp;<asp:Button ID="Button3" runat="server" Height="22px" onclick="Button3_Click" 
                        Text="CARGA DE TRABAJO POR ESTADO DE SOLICITUD" Width="328px" />
                    <br />
                    &nbsp;
                    <asp:Button ID="Button4" runat="server" 
                        Text="CARGA DE TRABAJO POR BUSQUEDA DE ESTADO" onclick="Button4_Click" />
                </td>
            </tr>
             <tr>
                <td align="center" colspan="3">
                <br />
              
                    <asp:Label ID="lblBuscar" runat="server" Font-Bold="True" Font-Size="Small" 
                        ForeColor="Black" Text="BUSCAR"></asp:Label>
                    &nbsp;<asp:DropDownList ID="ddlBuscar" runat="server" AutoPostBack="True" 
                        onselectedindexchanged="ddlBuscar_SelectedIndexChanged" Width="200px">
                        <asp:ListItem Value="0">POR:</asp:ListItem>
                        <asp:ListItem Value="1">FECHA </asp:ListItem>
                          <asp:ListItem Value="2">MUNICIPIO </asp:ListItem>
                    </asp:DropDownList>
                </td>
            </tr>
            <tr>
                <td align="center" colspan="3">
                    &nbsp; &nbsp; &nbsp;

                   
                    

                    <asp:Label ID="lblFechaInicioOrden" runat="server" Font-Bold="True" 
                        Font-Size="Small" ForeColor="Black" Text="FECHA INICIO" Visible="False"></asp:Label>
                    <asp:TextBox ID="txtFechaInicioOrden" runat="server" Visible="False"></asp:TextBox>
                    <asp:CalendarExtender ID="txtFechaInicioOrden_CalendarExtender" runat="server" 
                        Enabled="True" Format="dd/MM/yyyy" TargetControlID="txtFechaInicioOrden">
                    </asp:CalendarExtender>
                    <asp:Label ID="lblFechaFinOrden" runat="server" Font-Bold="True" 
                        Font-Size="Small" ForeColor="Black" Text="FECHA FIN" Visible="False"></asp:Label>
                    <asp:TextBox ID="txtFechaFinOrden" runat="server" Visible="False"></asp:TextBox>
                    <asp:CalendarExtender ID="txtFechaFinOrden_CalendarExtender" runat="server" 
                        Enabled="True" Format="dd/MM/yyyy" TargetControlID="txtFechaFinOrden">
                    </asp:CalendarExtender>
                       <asp:Label ID="LblMuni" runat="server" Font-Bold="True" 
                        Font-Size="Small" ForeColor="Black" Text="MUNICIPIO" Visible="False"></asp:Label>
                   
                    <asp:DropDownList ID="ddlMuni" runat="server">
                    </asp:DropDownList>
                   
                    <asp:Button ID="cmdAceptar" runat="server"
                        Text="ACEPTAR" Visible="False" onclick="cmdAceptar_Click1"  />
                </td>
            </tr>
        </table>

       
       

        <table runat="server" id="tbl_asignarPI">
            <tr>
                <td>
                    &nbsp;
               
                   <asp:Label ID="Label2" runat="server" Font-Bold="True" ForeColor="Black" 
                        Text="CARGA DE TRABAJO DE PERITOS"></asp:Label><asp:Label ID="lblNucPI" Font-Bold="True"
                            runat="server" Text=" "></asp:Label>
                </td>
            </tr>
            <tr>
             <td>
                        <asp:GridView ID="EstadisticaPI" runat="server" BackColor="White" 
                        BorderColor="#DEDFDE" BorderStyle="None" BorderWidth="1px" CellPadding="4" 
                        ForeColor="Black" GridLines="Vertical" Width="928px" DataKeyNames="ID_USUARIO"
                        AutoGenerateColumns="False" DataSourceID="dsCargaTrabajo" 
                        AllowPaging="True" onrowdatabound="EstadisticaPI_RowDataBound">
                        <AlternatingRowStyle BackColor="White" />
                        <Columns>
                        

                          

                          <asp:BoundField DataField="ID_USUARIO" HeaderText="ID_USUARIO" 
                                SortExpression="ID_USUARIO" Visible="True" >
                                <ItemStyle CssClass="hidden"/>
                                 <HeaderStyle CssClass="hidden"/>
                          </asp:BoundField>
                            <asp:BoundField DataField="PERITO_ASIGNADO" HeaderText="NOMBRE DEL PERITO" ReadOnly="True" 
                                SortExpression="PERITO_ASIGNADO" />
                            <asp:BoundField DataField="ASIGNADAS" HeaderText="ORDENES ASIGNADA" ReadOnly="True" 
                                SortExpression="ASIGNADAS" />
                            <asp:BoundField DataField="EN_PROCESO" HeaderText="ORDENES EN PROCESO" 
                                ReadOnly="True" SortExpression="EN_PROCESO" />
                            <asp:BoundField DataField="CUMPLIDAS" HeaderText="ORDENES CUMPLIDAS" ReadOnly="True" 
                                SortExpression="CUMPLIDAS" />
                            <asp:BoundField DataField="TOTAL" HeaderText="TOTAL DE ORDENES" ReadOnly="True" 
                                SortExpression="TOTAL" />
                        </Columns>
                        <FooterStyle BackColor="#CCCC99" />
                          <HeaderStyle BackColor="#006600" Font-Bold="True" ForeColor="White" 
                                                        HorizontalAlign="Center" />
                                                        <PagerStyle BackColor="#F7F7DE" ForeColor="Black" HorizontalAlign="Right" />
                                                        <RowStyle BackColor="#CCFFCC" 
                            HorizontalAlign="Center" />
                                                        <SelectedRowStyle BackColor="#CE5D5A" Font-Bold="True" ForeColor="White" />
                                                        <SortedAscendingCellStyle BackColor="#FBFBF2" />
                                                        <SortedAscendingHeaderStyle BackColor="#848384" />
                                                        <SortedDescendingCellStyle BackColor="#EAEAD3" />
                                                        <SortedDescendingHeaderStyle BackColor="#575357" />
                    </asp:GridView>           
                </td>
            
            </tr>
            <tr>
                <td>
                    &nbsp;
               
                    <asp:SqlDataSource ID="dsCargaTrabajo" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="Carga_Trabajo_Por_Perito" SelectCommandType="StoredProcedure">
                       <%-- <SelectParameters>
                            <asp:ControlParameter ControlID="IdUnidad" Name="IDUNIDAD" PropertyName="Text" 
                                Type="Int32" />
                        </SelectParameters>--%>
                    </asp:SqlDataSource>
                    
                </td>
            </tr>
        </table>



        <table runat="server" id="tbl_">
            <tr>
                <td  class="style3">
                  <asp:Chart ID="chIniciada" runat="server" DataSourceID="ds_Estadistica" 
                    Height="345px" Palette="EarthTones" Width="926px" >
                    <Series>
                        <asp:Series IsValueShownAsLabel="True" Name="Series1" XValueMember="PERITO_ASIGNADO" Font="Arial, 5.25pt, style=Bold" 
                            YValueMembers="TOTAL" ChartType="StackedColumn">
                        </asp:Series>
                    </Series>
                    <ChartAreas>
                        <asp:ChartArea Name="ChartArea1">
                            <Area3DStyle Enable3D="True" />
                        </asp:ChartArea>
                    </ChartAreas>
                    <Titles>
                        <asp:Title Name="Title1" Text="CARGA DE TRABAJO DE PERITOS" Font="Arial, 10pt, style=Bold" ForeColor="#006600">
                        </asp:Title>
                    </Titles>

                </asp:Chart>
                </td>
            </tr>
            <tr>
                <td>
                <asp:SqlDataSource ID="ds_Estadistica" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="Carga_Trabajo_Por_Perito" SelectCommandType="StoredProcedure">
                   <%-- <SelectParameters>
                        <asp:ControlParameter ControlID="IdUnidad" Name="IDUNIDAD" PropertyName="Text" 
                            Type="Int32" />
                    </SelectParameters>--%>
                    </asp:SqlDataSource>
                </td>
            </tr>
            
        </table>
       

         <table runat="server" id="TLB2">
            <tr>
                <td>
                    &nbsp;
               
                   <asp:Label ID="Label5" runat="server" Font-Bold="True" ForeColor="Black" 
                        Text="CARGA DE TRABAJO POR DEPARTAMENTOS"></asp:Label><asp:Label ID="Label6" Font-Bold="True"
                            runat="server" Text=" "></asp:Label>
                </td>
            </tr>
            <tr>
             <td>
                        <asp:GridView ID="GridView2" runat="server" BackColor="White" 
                        BorderColor="#DEDFDE" BorderStyle="None" BorderWidth="1px" CellPadding="4" 
                        ForeColor="Black" GridLines="Vertical" Width="928px"
                        AutoGenerateColumns="False" DataSourceID="dsCargaTrabajoDptoE" 
                        AllowPaging="True" onrowdatabound="GridView1_RowDataBound">
                        <AlternatingRowStyle BackColor="White" />
                        <Columns>
                        

                          

                        
                            <asp:BoundField DataField="DEPTO_PRCIAL" HeaderText="DEPTO_PRCIAL" 
                                SortExpression="DEPTO_PRCIAL" />
                            <asp:BoundField DataField="PENDIENTE" HeaderText="PENDIENTE" ReadOnly="True" 
                                SortExpression="PENDIENTE" />
                            <asp:BoundField DataField="ASIGNADAS" HeaderText="ASIGNADAS" 
                                ReadOnly="True" SortExpression="ASIGNADAS" />
                            <asp:BoundField DataField="EN_PROCESO" HeaderText="EN_PROCESO" ReadOnly="True" 
                                SortExpression="EN_PROCESO" />
                            <asp:BoundField DataField="CUMPLIDAS" HeaderText="CUMPLIDAS" ReadOnly="True" 
                                SortExpression="CUMPLIDAS" />
                                <asp:BoundField DataField="TOTAL" HeaderText="TOTAL" ReadOnly="True" 
                                SortExpression="TOTAL" />
                        </Columns>
                        <FooterStyle BackColor="#CCCC99" />
                          <HeaderStyle BackColor="#006600" Font-Bold="True" ForeColor="White" 
                                                        HorizontalAlign="Center" />
                                                        <PagerStyle BackColor="#F7F7DE" ForeColor="Black" HorizontalAlign="Right" />
                                                        <RowStyle BackColor="#CCFFCC" 
                            HorizontalAlign="Center" />
                                                        <SelectedRowStyle BackColor="#CE5D5A" Font-Bold="True" ForeColor="White" />
                                                        <SortedAscendingCellStyle BackColor="#FBFBF2" />
                                                        <SortedAscendingHeaderStyle BackColor="#848384" />
                                                        <SortedDescendingCellStyle BackColor="#EAEAD3" />
                                                        <SortedDescendingHeaderStyle BackColor="#575357" />
                    </asp:GridView>           
                </td>
            
            </tr>
            <tr>
                <td>
                    &nbsp;
               
                    <asp:SqlDataSource ID="dsCargaTrabajoDptoE" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="SOL_POR_DEPARTAMENTO_ESTADO" SelectCommandType="StoredProcedure">
                    </asp:SqlDataSource>
                    
                </td>
            </tr>
        </table>
         <table runat="server" id="TL2">
            <tr>
                <td  class="style3">
                  <asp:Chart ID="Chart2" runat="server" DataSourceID="EsCargaTrabajoDptoE" 
                    Height="345px" Palette="EarthTones" Width="926px" >
                    <Series>
                        <asp:Series IsValueShownAsLabel="True" Name="Series1" XValueMember="DEPTO_PRCIAL" Font="Arial, 8.25pt, style=Bold" 
                            YValueMembers="TOTAL" ChartType="StackedColumn">
                        </asp:Series>
                       
                    </Series>
                    <ChartAreas>
                        <asp:ChartArea Name="ChartArea2">
                            <Area3DStyle Enable3D="True" />
                        </asp:ChartArea>
                    </ChartAreas>
                    <Titles>
                        <asp:Title Name="Title1" Text="CARGA DE TRABAJO POR DEPARTAMENTOS" Font="Arial, 10pt, style=Bold" ForeColor="#006600">
                        </asp:Title>
                    </Titles>

                </asp:Chart>
                </td>
            </tr>
            <tr>
                <td>
                <asp:SqlDataSource ID="EsCargaTrabajoDptoE" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="SOL_POR_DEPARTAMENTO_ESTADO" SelectCommandType="StoredProcedure">
                    </asp:SqlDataSource>
                </td>
            </tr>
            
        </table>

         
         <table runat="server" id="Table1">
            <tr>
                <td>
                    &nbsp;
               
                   <asp:Label ID="Label4" runat="server" Font-Bold="True" ForeColor="Black" 
                        Text="CARGA DE TRABAJO DE PERITOS POR DEPARTAMENTOS"></asp:Label><asp:Label ID="Label7" Font-Bold="True"
                            runat="server" Text=" "></asp:Label>
                </td>
            </tr>
            <tr>
             <td>
                        <asp:GridView ID="gvBusquedaP" runat="server" BackColor="White" 
                        BorderColor="#DEDFDE" BorderStyle="None" BorderWidth="1px" CellPadding="4" 
                        ForeColor="Black" GridLines="Vertical" Width="928px"
                        AutoGenerateColumns="False" DataSourceID="dsCargaBusqueda" 
                        AllowPaging="True" onrowdatabound="GridView1_RowDataBound">
                        <AlternatingRowStyle BackColor="White" />
                        <Columns>
                        

                          

                        
                            <asp:BoundField DataField="DEPTO_PRCIAL" HeaderText="DEPTO_PRCIAL" 
                                SortExpression="DEPTO_PRCIAL" />
                            <asp:BoundField DataField="PENDIENTE" HeaderText="PENDIENTE" ReadOnly="True" 
                                SortExpression="PENDIENTE" />
                            <asp:BoundField DataField="ASIGNADAS" HeaderText="ASIGNADAS" 
                                ReadOnly="True" SortExpression="ASIGNADAS" />
                            <asp:BoundField DataField="EN_PROCESO" HeaderText="EN_PROCESO" ReadOnly="True" 
                                SortExpression="EN_PROCESO" />
                            <asp:BoundField DataField="CUMPLIDAS" HeaderText="CUMPLIDAS" ReadOnly="True" 
                                SortExpression="CUMPLIDAS" />
                                <asp:BoundField DataField="TOTAL" HeaderText="TOTAL" ReadOnly="True" 
                                SortExpression="TOTAL" />
                        </Columns>
                        <FooterStyle BackColor="#CCCC99" />
                          <HeaderStyle BackColor="#006600" Font-Bold="True" ForeColor="White" 
                                                        HorizontalAlign="Center" />
                                                        <PagerStyle BackColor="#F7F7DE" ForeColor="Black" HorizontalAlign="Right" />
                                                        <RowStyle BackColor="#CCFFCC" 
                            HorizontalAlign="Center" />
                                                        <SelectedRowStyle BackColor="#CE5D5A" Font-Bold="True" ForeColor="White" />
                                                        <SortedAscendingCellStyle BackColor="#FBFBF2" />
                                                        <SortedAscendingHeaderStyle BackColor="#848384" />
                                                        <SortedDescendingCellStyle BackColor="#EAEAD3" />
                                                        <SortedDescendingHeaderStyle BackColor="#575357" />
                    </asp:GridView>           
                </td>
            
            </tr>
            <tr>
                <td class="style4">
                    &nbsp;<asp:GridView ID="gvBusquedaP0" runat="server" AllowPaging="True" 
                        AutoGenerateColumns="False" BackColor="White" BorderColor="#DEDFDE" 
                        BorderStyle="None" BorderWidth="1px" CellPadding="4" DataSourceID="DSBUSQUEDA" 
                        ForeColor="Black" GridLines="Vertical" onrowdatabound="GridView1_RowDataBound" 
                        Width="928px">
                        <AlternatingRowStyle BackColor="White" />
                        <Columns>
                            <asp:BoundField DataField="DEPTO_PRCIAL" HeaderText="DEPTO_PRCIAL" 
                                SortExpression="DEPTO_PRCIAL" />
                            <asp:BoundField DataField="PENDIENTE" HeaderText="PENDIENTE" ReadOnly="True" 
                                SortExpression="PENDIENTE" />
                            <asp:BoundField DataField="ASIGNADAS" HeaderText="ASIGNADAS" ReadOnly="True" 
                                SortExpression="ASIGNADAS" />
                            <asp:BoundField DataField="EN_PROCESO" HeaderText="EN_PROCESO" ReadOnly="True" 
                                SortExpression="EN_PROCESO" />
                            <asp:BoundField DataField="CUMPLIDAS" HeaderText="CUMPLIDAS" ReadOnly="True" 
                                SortExpression="CUMPLIDAS" />
                            <asp:BoundField DataField="TOTAL" HeaderText="TOTAL" ReadOnly="True" 
                                SortExpression="TOTAL" />
                        </Columns>
                        <FooterStyle BackColor="#CCCC99" />
                        <HeaderStyle BackColor="#006600" Font-Bold="True" ForeColor="White" 
                            HorizontalAlign="Center" />
                        <PagerStyle BackColor="#F7F7DE" ForeColor="Black" HorizontalAlign="Right" />
                        <RowStyle BackColor="#CCFFCC" HorizontalAlign="Center" />
                        <SelectedRowStyle BackColor="#CE5D5A" Font-Bold="True" ForeColor="White" />
                        <SortedAscendingCellStyle BackColor="#FBFBF2" />
                        <SortedAscendingHeaderStyle BackColor="#848384" />
                        <SortedDescendingCellStyle BackColor="#EAEAD3" />
                        <SortedDescendingHeaderStyle BackColor="#575357" />
                    </asp:GridView>
&nbsp;<asp:SqlDataSource ID="dsCargaBusqueda" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="SOL_POR_DEPARTAMENTO_ESTADO" SelectCommandType="StoredProcedure">
                    </asp:SqlDataSource>
                    <asp:SqlDataSource ID="DSBUSQUEDA1" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="BUSCA_SOL_POR_DEPARTAMENTO_ESTADO" 
                        SelectCommandType="StoredProcedure">
                        <SelectParameters>
                             
                             <asp:ControlParameter ControlID="ddlMuni" Name="idmuncipio" PropertyName="SelectedValue" 
                                Type="Int32" DefaultValue="" />
                            
                            <asp:Parameter DefaultValue=" " Name="FECHAINICIAL" Type="String" />
                            <asp:Parameter DefaultValue=" " Name="FECHAFINAL" Type="String" />
                        </SelectParameters>
                     
                    </asp:SqlDataSource>
                    <asp:SqlDataSource ID="DSBUSQUEDA" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="BUSCA_SOL_POR_DEPARTAMENTO_ESTADO" 
                        SelectCommandType="StoredProcedure">
                        <SelectParameters>
                            <asp:Parameter DefaultValue="0" Name="idmuncipio" Type="Int32" />
                            <asp:ControlParameter ControlID="txtFechaInicioOrden"
                                Name="FECHAINICIAL" PropertyName="Text" Type="String" />
                            <asp:ControlParameter ControlID="txtFechaFinOrden" Name="FECHAFINAL"
                                PropertyName="Text" Type="String" />
                        </SelectParameters>
                    </asp:SqlDataSource>

                </td>
            </tr>
        </table>
         <table runat="server" id="Table2">
            <tr>
                <td  class="style3">
                    <asp:Chart ID="Chart3" runat="server" DataSourceID="EsCargaBusqueda" 
                        Height="345px" Palette="EarthTones" Width="926px">
                        <Series>
                            <asp:Series ChartType="StackedColumn" Font="Arial, 8.25pt, style=Bold" 
                                IsValueShownAsLabel="True" Name="Series1" XValueMember="DEPTO_PRCIAL" 
                                YValueMembers="TOTAL">
                            </asp:Series>
                        </Series>
                        <ChartAreas>
                            <asp:ChartArea Name="ChartArea3">
                                <Area3DStyle Enable3D="True" />
                            </asp:ChartArea>
                        </ChartAreas>
                        <Titles>
                            <asp:Title Font="Arial, 10pt, style=Bold" ForeColor="#006600" Name="Title1" 
                                Text="CARGA DE TRABAJO POR DEPARTAMENTOS">
                            </asp:Title>
                        </Titles>
                    </asp:Chart>
                </td>
            </tr>
            <tr>
                <td>
                    <asp:SqlDataSource ID="EsCargaBusqueda" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:PGJ_NSJPConnectionString2 %>" 
                        SelectCommand="SOL_POR_DEPARTAMENTO_ESTADO" SelectCommandType="StoredProcedure">
                    </asp:SqlDataSource>
                    <br />
                </td>

            </tr>
            
        </table>
    </ContentTemplate>
    </asp:UpdatePanel>

    
    
    
</div>

</asp:Content>
